...
def create_synthetic_data(n_samples=10):
    import numpy as np
    np.random.seed(0)
    # Generate synthetic data (ensure the ranges are realistic for the features)
    rooms = np.random.randint(1, 10, n_samples)  # Random number of rooms
    landsize = np.random.randint(50, 1000, n_samples)  # Random land size
    year_built = np.random.randint(1800, 2023, n_samples)  # Random year built
...
# Display predictions
print_separator("Predictions for New Data")
for i, prediction in enumerate(new_predictions):
    print(f"Data Point {i+1}: {new_data.iloc[i].to_dict()}, Predicted Price: {prediction}")